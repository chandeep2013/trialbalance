...
function(Controller, MessageBox, Token, BusyIndicator, formatter, Dialog, MessageToast, JSONModel) {
    ...
    return Controller.extend("com.tr.trialbalance.controller.OTP", {
        ...
        onAfterRendering: function() {
            ...
            ///----------- Set DateFormat for EndUsers -----------------//////
            var userLang = navigator.language || navigator.userLanguage;
            const sample = window.Intl ? new Intl.DateTimeFormat(userLang, {
            ...
            r[mm] = 'MM';
            r[dd] = 'dd';
            this.getView().byId("fromDate").setDisplayFormat(r.join(sample.match(/[-.]/) || '/'));
            this.getView().byId("toDate").setDisplayFormat(r.join(sample.match(/[-.]/) || '/'));
        },
                ...
                function(token) {
                    $.ajax({
                        ...
                        error: function(errorThrown) {
                            ...
                            if (errorThrown.status === 400 && errorThrown.responseJSON.error && errorThrown.responseJSON.error.Message) {
                                errormsg = errorThrown.responseJSON.error.Message;
                                MessageBox.success(errormsg);
                                return;
                            } else if (errorThrown.status === 400 && errorThrown.responseJSON.response.Message) {
                                errormsg = errorThrown.responseJSON.response.Message;
                                MessageBox.success(errormsg);
                                return;
                            } else if (errorThrown.status === 504) {
